...
#此程式用於爬取各隊指定守備位置於一軍之打擊基礎、進階數據並存放於SQLite資料庫中
...
try:
    ...
    chrome.find_element_by_xpath('//*[@id="DefenceType"]/option[6]').click()
    Position = chrome.find_element_by_xpath('//*[@id="DefenceType"]/option[6]').text
    time.sleep(2)
    ...
    for i in range(1, len(trs)):
        ...
        cur.execute('insert into Position_Batter(PBatter_Position,PBatter_name , PBatter_YearTeam , PBatter_Game, PBatter_PA, PBatter_AB, PBatter_R,'
                    ...
                    '?,?,?,?,?,?,?,?)',
                    (
                    Position,Player, Year, Game, PA, AB, R, RBI, K, BB, HBP, SF, SH, H, H1, H2, H3, HR, TB, AVG, OBP, SLG, OPS,
